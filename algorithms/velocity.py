...
class VelocityTracker:
    ...
    def point_check(self, fps, fcnt, id, center, frame, bbox):
        if id not in self.section:
            self.section[id] = 1
        elif is_crossed(id,center, self.startLine[0], self.startLine[1]) and self.section[id] == 1:
            self.section[id] += 1
            ...
            cv2.putText(frame, str(self.velocity[id])+"km/h", (int(bbox[2]), int(bbox[3])), 0, 5e-3 * 150, (255, 0, 255), 2)
